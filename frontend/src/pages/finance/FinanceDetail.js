...
import {
  ...
  getDepCustomer,
  setDepositorMoney,
} from "redux/Depositor";
import {
  makeSavingser,
  setSavingserFinancialProductId,
  getSavCustomer,
  setSavingserRegularMoney,
} from "redux/Savingser";
import {
  makeLoaner,
  setLoanerFinancialProductId,
  getLoaCustomer,
  setLoanerMoney,
} from "redux/Loaner";
...
const FinanceDetail = () => {
  ...
  const setBase = () => {
    dispatch(getProductInfo(params.id)).then(async (product) => {
      if (product.payload.data === null) nav("/financial");
      await setFinance(product.payload.data);
      switch (
        product.payload.data === null ? "" : product.payload.data.productType
      ) {
        case "DEPOSIT":
          dispatch(setDepositorFinancialProductId(product.payload.data.id));
          if (role === "ROLE_PARENT") {
            dispatch(getDepCustomer(product.payload.data.id)).then((res) => {
        ...
        case "SAVINGS":
          ...
          if (role === "ROLE_PARENT") {
            dispatch(getSavCustomer(product.payload.data.id)).then((res) => {
        ...
        case "LOAN":
          ...
          if (role === "ROLE_PARENT") {
            dispatch(getLoaCustomer(product.payload.data.id)).then((res) => {
  ...
  const makeD = () => {
    dispatch(makeDepositor(depositor)).then((resultAction) => {
  ...
  const makeS = () => {
    dispatch(makeSavingser(savingser)).then((resultAction) => {
  ...
  const makeL = () => {
    dispatch(makeLoaner(loaner)).then((resultAction) => {
      console.log(loaner);
  ...
  const handleRateChange = (event) => {
    ...
    else dispatch(setSavingserRegularMoney(Number(event.target.value)));
  };
  return (
    finance.name !== undefined && (
      <div className="product-detail-container">
        <div className="product-detail-header">
          ...
          <div className="product-summary-infomation">
            <img
              ...
              src={
                finance.productType === "DEPOSIT"
                  ? Deposit
                  : finance.productType === "SAVINGS"
                  ? Savings
                  : Loan
              }
              alt="상품"
        ...
        <div className="product-info-container">
            ...
            <div className="detail-dropdown">
              <div className="flex-row">
                ...
                <div className="text-label" style={{ width: "46%" }}>
                  {productName[finance.productType]}
                </div>
              </div>
              <div className="flex-row">
                <MdLabelOutline className="finance-detail-icon" />
              ...
              <div className="flex-row">
                <MdLabelOutline className="finance-detail-icon" />
                <div className="text-label" style={{ width: "40%" }}>
                  상품 {finance.productType === "LOAN" ? "이자" : "이율"} :
                  {"    "}
                </div>
                <div className="text-label" style={{ width: "46%" }}>
                  {finance.rate}%
          ...
          {showDiv2 && (
            <div className="detail-dropdown">
              <div className="flex-row">
                ...
                <div className="text-label join-essential-info">
                  {finance.info}
                </div>
              </div>
            </div>
          ...
          <div className="product-space-between">
            <span className="option-title">
              {productName[finance.productType]} 설명서 확인
            </span>
            <div className="option-buttons">
              {role === "ROLE_PARENT" ? null : (
                <>
        ...
        {role === "ROLE_PARENT" ? null : (
          <div className="product-apply-button" onClick={apply}>
            상품 신청
          </div>
        )}
        {role === "ROLE_PARENT" ? (
          <ApplyList applys={applys} type={finance.productType} />
        ) : null}
