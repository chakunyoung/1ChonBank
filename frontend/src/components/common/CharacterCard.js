...
function CharacterCard() {
  ...
  useEffect(() => {
    ...
    } else image.src = characters[user.characterNum];
    // 이미지가 로드되면 실행할 함수
    image.onload = () => {
  ...
  const handlePrevCharacter = () => {
    const newCharacterNum =
      (user.characterNum - 1 + characters.length) % characters.length;
  ...
  return (
    <div className="charactercard-container">
      <button className="left-button" onClick={handlePrevCharacter}>
        <IoIosArrowDropleft />
      </button>
      <div className="character-image">
        <img
          className="charctercard-imagearea"
          src={characterImage}
          alt={`Character ${user.characterNum + 1}`}
        />
      </div>
      <button className="right-button" onClick={handleNextCharacter}>
        <IoIosArrowDropright />
      </button>
    </div>
  );
